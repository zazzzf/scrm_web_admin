<template>
	<div>
		...
		<div style="min-height: 500px;">
			<el-tag style="cursor: pointer; margin:10px"  type="success" @click='editThisTag(tag)'
	...
	methods: {
		...
		handleDel(categoryId) {
			this.$confirm('此操作将永久删除该分类, 是否继续?', '提示', {
			  ...
			  categoryApi.delCategory({ categoryId }).then(res => {
		...
		handleInputConfirm() {
			...
			if (inputValue) {
				categoryApi.addCategory({name:this.inputValue}).then(res => {
		...
		editThisTag(data){
			this.$prompt('请输入分类名', '提示', {
			  ...
			  categoryApi.editCategory({categoryId: data.category_id,name:value}).then(res => {
